...
TMP_TOKEN = std::make_unique<std::string>(PayloadDTO::getTokenPrefix() + JWTUtil::generateTokenByRsa(payload, RSA_PRIV_KEY->c_str()));
...
#else
	request->putHeaderIfNotExists(API_H_TOKEN, TMP_TOKEN->c_str());
#endif
	return nullptr;
}
